...
- `/bench` is the internal operator surface (propose, ratify, adjudicate, upload, and the `/bench/claims` claiming queue). It sits behind `require_authenticated_user` + `require_operator_user` (plus the matching `live_session` on_mounts). Don't add user-facing affordances to it — owner surfaces are their own routes.
- Artifact bytes are served by `/bench/artifacts/:id`, inside the operator gate. Serving artifact images on a public page is blocked on a rights call; possession proofs are stored `visibility: :private` and read at the bench only.
...
- The operator flag is set out of band — `priv/repo/seeds.exs` creates the first operator (`grolsen@gmail.com`, override with `SEED_OPERATOR_EMAIL`), and `SantoApi.Accounts.set_operator(user, true)` from IEx handles the rest. There is no self-serve path until the user admin surface (§9.2) exists.
