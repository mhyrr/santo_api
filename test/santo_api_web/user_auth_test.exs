defmodule SantoApiWeb.UserAuthTest do
  ...
  @remember_me_cookie "_santo_api_web_user_remember_me"
  ...
  describe "log_in_user/3" do
    test "stores the user token in the session", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == ~p"/garage"
    ...
    test "redirects to the garage when user is already logged in", %{conn: conn, user: user} do
      conn =
        conn
        |> assign(:current_scope, Scope.for_user(user))
        |> UserAuth.log_in_user(user)
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      SantoApiWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "require_operator_user/2" do
    setup %{conn: conn} do
      %{conn: UserAuth.fetch_current_scope_for_user(conn, [])}
    end

    test "redirects if there is no logged in user", %{conn: conn} do
      conn = conn |> fetch_flash() |> UserAuth.require_operator_user([])

      assert conn.halted
      assert redirected_to(conn) == ~p"/users/log-in"
    end

    test "redirects if the user is not an operator", %{conn: conn, user: user} do
      conn =
        conn
        |> assign(:current_scope, Scope.for_user(user))
        |> fetch_flash()
        |> UserAuth.require_operator_user([])
    ...
    test "does not redirect operators", %{conn: conn} do
      operator = operator_fixture()

      conn =
        conn
        |> assign(:current_scope, Scope.for_user(operator))
        |> UserAuth.require_operator_user([])
  ...
  describe "on_mount :require_operator" do
    test "assigns current_scope and continues for operators", %{conn: conn} do
      operator = operator_fixture()
      user_token = Accounts.generate_user_session_token(operator)
      session = conn |> put_session(:user_token, user_token) |> get_session()

      {:cont, updated_socket} =
        UserAuth.on_mount(:require_operator, %{}, session, %LiveView.Socket{})

      assert updated_socket.assigns.current_scope.user.id == operator.id
    end

    test "halts for a logged in non-operator", %{conn: conn, user: user} do
      user_token = Accounts.generate_user_session_token(user)
      session = conn |> put_session(:user_token, user_token) |> get_session()
      ...
      {:halt, updated_socket} = UserAuth.on_mount(:require_operator, %{}, session, socket)
      assert updated_socket.assigns.flash["error"] =~ "not authorized"
    end

    test "halts when there is no logged in user" do
      socket = %LiveView.Socket{
        endpoint: SantoApiWeb.Endpoint,
        assigns: %{__changed__: %{}, flash: %{}}
      }

      {:halt, updated_socket} = UserAuth.on_mount(:require_operator, %{}, %{}, socket)
      assert updated_socket.assigns.current_scope == nil
    end
  end
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        SantoApiWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
