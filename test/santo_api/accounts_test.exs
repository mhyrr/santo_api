defmodule SantoApi.AccountsTest do
  use SantoApi.DataCase

  alias SantoApi.Accounts

  import SantoApi.AccountsFixtures
  alias SantoApi.Accounts.{User, UserToken}
  ...
  describe "set_operator/2" do
    test "users are not operators by default" do
      refute user_fixture().operator
    end

    test "grants and revokes the operator flag" do
      user = user_fixture()

      assert {:ok, user} = Accounts.set_operator(user, true)
      assert user.operator
      assert Repo.get!(User, user.id).operator

      assert {:ok, user} = Accounts.set_operator(user, false)
      refute user.operator
      refute Repo.get!(User, user.id).operator
    end
  end
