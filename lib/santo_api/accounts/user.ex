defmodule SantoApi.Accounts.User do
  use Ecto.Schema
  ...
  defp validate_email(changeset, opts) do
    ...
    if Keyword.get(opts, :validate_unique, true) do
      changeset
      |> unsafe_validate_unique(:email, SantoApi.Repo)
  ...
  @doc """
  A user changeset for granting or revoking the operator flag.

  Operators are the only users who may reach /bench. There is no self-serve
  path to this — it is set out of band (see `SantoApi.Accounts.set_operator/2`).
  """
  ...
  """
  def valid_password?(%SantoApi.Accounts.User{hashed_password: hashed_password}, password)
